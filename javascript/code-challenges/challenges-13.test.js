...
// Solution code here...
  let longest = arr.reduce((acc, curVal) => acc.length > curVal.length ? acc : curVal, 0);
...
const standardizePhoneNumbers = (arr) => {
  ...
  return arr.map((number) => {
    let formattedNum = number.substring(1, 4) + number.substring(6, 9) + number.substring(10);
    return formattedNum;
  });
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(arr2 => arr2.filter(string => !string.includes('Brook')));
};
...
const sortByDay = (arr) => {
  ...
  for (let i = 0; i < daysOfWeek.length; i++) {
    strArr[i] = arr.filter(string => string.includes(daysOfWeek[i]));
  }
  return strArr;
};
