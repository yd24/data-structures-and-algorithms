...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map((word) => {
    let str = word.substring(1, word.length);
    str = word.substring(0, 1).toUpperCase() + str;
    return str;
  });
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let names = arr.reduce((acc, curVal) => acc += (Number(curVal.mass) > Number(arr[0].mass) ? (curVal.name + ' - ') : ''), '');
