...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  return arr.filter(value => forbiddenValues.includes(value) === false);
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  const newArr = getBaseStatGreaterThan(arr, minBaseStat);
  return newArr.map(stat => stat.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  return arr.filter(person => !Object.prototype.hasOwnProperty.call(person, 'children'));
};
...
const evenOddNumericValues = (arr) => {
  ...
  return filteredArr.map(num => num % 2 === 0 ? 'even' : 'odd');
};
