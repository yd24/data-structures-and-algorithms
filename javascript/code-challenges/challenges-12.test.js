...
const validateEmail = (email) => {
  // Solution code here...
  let regex = /^[A-Za-z0-9]+(?:.?[A-Za-z0-9]+)?@{1}\w+(?:.com+|.org+|.net+)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let regex = /^((\({1}\d{3}\){1})|(\d{3}))(-?\s?)\d{3}(-?|\s?)\d{4}$/;
...
const findTagNames = elements => {
  // Solution code here...
  let regex = /\/\w+/g;
  return elements.toString().match(regex);
};
