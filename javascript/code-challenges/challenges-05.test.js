...
const calculateAverage = (arr) => {
  // Solution code here...
  let avgObj = arr.reduce((accumulator, value, idx) => {
    accumulator.sum = accumulator.sum + value;
    accumulator.count = idx + 1;
    return accumulator;
  }, { count: 0, sum: 0 });
  return avgObj.sum / avgObj.count;
};
...
const extractStats = (snorlaxData) => {
  // Solution code here...
  return snorlaxData.stats.reduce((accumulator, value) => {
...
const extractChildren = (arr) => {
  // Solution code here...
  return arr.filter(char => char.name.toLowerCase().includes('a')).reduce((accumulator, value) => {
    if (Object.prototype.hasOwnProperty.call(value, 'children')) {
